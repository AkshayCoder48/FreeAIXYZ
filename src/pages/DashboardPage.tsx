...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchProjects = async () => {
      ...
      const { data: owned, error: ownedError } = await supabase
        .from('projects')
        .select('id, user_id, title, preview_url, hosting_site_name, created_at, updated_at, starred')
        .eq('user_id', user.id)
  ...
  const deployedProject = projects.find((p) => p.hosting_site_name)
