    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3">
        <div className="relative">
          ...
          <Input
            placeholder="Search image models (e.g. anime, flux, meinamix, nsfw)…"
            value={query}
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((m) => {
          ...
          return (
            <div
              key={m.id}
              className="group rounded-xl border border-border bg-card/40 backdrop-blur p-4 hover:border-[#ff9a3c]/50 hover:bg-[#ff9a3c]/5 transition-all"
            >
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-[11px] text-muted-foreground pt-2 border-t border-border/50">
        ...
        <span className="flex items-center gap-1.5">
          <ImageIcon className="h-3.5 w-3.5 text-blue-500" /> realism
        </span>
        <span className="flex items-center gap-1.5">
          <ShieldAlert className="h-3.5 w-3.5 text-rose-500" /> NSFW (pass{" "}
